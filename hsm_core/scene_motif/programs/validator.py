...
from .interpretor import execute
from typing import Optional

load_dotenv()
from hsm_core.config import PROJECT_ROOT
# smc_prompts_path = str(project_root / "configs" / "prompts" / "smc_prompts.yaml")
...
def _map_objects(objs1: list[Obj], objs2: list[Obj]) -> tuple[dict[Obj, Obj], np.ndarray]:
    ...
    distances = np.ones(len(objs1)) * float("inf")
...
def validate_syntax(program: Program, require_objs: bool = False, require_arrangements: bool = False, variables: Optional[dict] = None) -> tuple[bool, str]:
    '''
    Evaulate whether the syntax of the program is valid and if it produces expected output variables.

    Args:
        program (Program): The program to validate. This is typically a meta-program
            with an appended line like `objs = <llm_generated_function_call>`.
        require_objs (bool): Whether the program must produce an "objs" variable in its execution scope.
            Defaults to True.
        require_arrangements (bool): Whether the program must produce an "arrangements" variable.
            Defaults to False.
        variables (dict, optional): A dictionary of pre-defined variables available during program execution
            (e.g., for sub-arrangements). Defaults to None.
        
    Returns:
        tuple[bool, str]: A tuple containing:
            - valid (bool): True if the program is valid and meets requirements, False otherwise.
            - error_message (str): An error message if invalid, empty string otherwise.
    '''
    try:
        variable_dict = execute(program, variables or {})
        ...
        if require_arrangements and "arrangements" not in variable_dict:
            return False, "The program is missing an list named `arrangements`. You should append all arrangements to it.\n"
        return True, ""
    except SyntaxError as e:
        # Extract the problematic line and show context
        lines = program.code_string.split('\n')
        lineno = e.lineno if e.lineno is not None else 1
        context_start = max(0, lineno - 2)
        context_end = min(len(lines), lineno + 2)
        code_context = '\n'.join(f"{i+1}: {line}" for i, line in enumerate(lines[context_start:context_end]))
...
#         error_message: str, the error message if the program is invalid (used for reprompting)
#     '''

#     # Define the response validation function
...
#     gpt_validator = gpt.Session(smc_prompts_path)
...
def validate_num_objects(program: Program, ground_truth_objs: list[Obj]) -> tuple[bool, str]:
    ...
    Returns:
        ...
        error_message: str, the error message if the number of objects is invalid (used for reprompting)
    '''
    ...
    for obj in ground_truth_objs:
        ground_truth_obj_dict.setdefault(obj.label, []).append(obj)
    
    # Check if the number of objects is the same for each object label
    for label, objs in ground_truth_obj_dict.items():
        if label not in program_obj_dict:
            error_message += f"The original program has {len(objs)} {label}, but there are none in your program.\n"
            valid = False
        elif len(objs) != len(program_obj_dict[label]):
            error_message += f"The original program has {len(objs)} {label}, but your program created {len(program_obj_dict[label])} {label}.\n"
            valid = False

            # Identify if there are duplicated objects
            if len(program_obj_dict[label]) > len(objs):
                for obj in objs:
                    match_count = 0
                    for program_obj in program_obj_dict[label]:
                        if np.all(np.isclose(obj.bounding_box.centroid, program_obj.bounding_box.centroid)):
                            match_count += 1
                    if match_count > 1:
                        error_message += f"The ground truth {label} at {obj.bounding_box.centroid.tolist()} is duplicated in your program.\n"                
...
def validate_centroids(program: Program, ground_truth_objs: list[Obj], centroid_threshold: float = 0.01) -> tuple[bool, str]:
    ...
    Returns:
        ...
        error_message: str, the error message if the centroids are invalid (used for reprompting)
    '''
...
def validate_bounding_boxes(program: Program, 
    ...
    Returns:
        ...
        error_message: str, the error message if the bounding boxes are invalid (used for reprompting)
    '''
...
def validate_meta_program(meta_program: Program, function_calls: list[str], reference_programs: list[Program]) -> tuple[bool, str]:
    ...
    Returns:
        valid: bool, whether the meta-program is valid
    ...
    for reference_idx, reference_program in enumerate(reference_programs):
        ...
        reference_program_variable_dict = execute(reference_program)
