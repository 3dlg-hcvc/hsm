...
class SceneMotif:
    ...
    def get_mesh_ids(self, object_names: Optional[list[str]] = None) -> dict:
        ...
        for obj in objects:
            try:
                ...
                print(f"Error retrieving mesh id for {obj.name}: {e}")
        return ids
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        for obj in self.objects:
            if not obj:
                print(f"Warning: Skipping None object in motif {self.id}")
                continue
                
            # Ensure object has required fields
            if not all(hasattr(obj, field) for field in ['name', 'position', 'dimensions', 'rotation', 'mesh_path']):
                print(f"Warning: Object {obj.name if hasattr(obj, 'name') else 'unknown'} missing required fields")
                continue
