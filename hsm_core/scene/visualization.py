...
from .grid_utils import create_grid, calculate_free_space
from .room_geometry import create_custom_room
...
def export_scene_glb(scene: Any, output_path: Path, recreate_scene: bool = False) -> None:
    ...
    """
    try:
        scene.export(output_path, recreate_scene=recreate_scene)
        print(f"Scene GLB exported to: {output_path}")
    except Exception as e:
        print(f"Error exporting scene GLB: {e}")
        raise
...
def export_scene_state_to_json(scene: Any, output_path: Path) -> None:
    ...
    """
    try:
        scene.save_state(output_path)
        print(f"Scene state exported to: {output_path}")
    except Exception as e:
        print(f"Error exporting scene state: {e}")
        raise
...
def load_trimesh_scene(glb_path: str, verbose: bool = False) -> trimesh.Scene:
    if verbose:
        print(f"\nLoading GLB scene from: {glb_path}")
    ...
    if verbose:
        ...
        print("----------------------")
        print(f"Base frame: {scene.graph.base_frame}")
        print(f"Number of nodes: {len(scene.graph.nodes)}")
        print(f"Geometry nodes: {scene.graph.nodes_geometry}")
        print("\nGeometry items:")
    ...
    for node_name in scene.graph.nodes_geometry:
        ...
        try:
            ...
            if verbose:
                print(f"Transform found for {node_name}")
        except Exception as e:
            if verbose:
                print(f"Error getting transform for {node_name}: {e}")
    ...
    for geom_name in scene.geometry.keys():
        if geom_name not in transforms:
            if verbose:
                print(f"\nProcessing duplicate geometry: {geom_name}")
            original_name = geom_name.split('_')[0]
            if original_name in transforms:
                transforms[geom_name] = transforms[original_name].copy()
                if verbose:
                    print(f"Using transform from {original_name} for {geom_name}")
            else:
                transforms[geom_name] = np.eye(4)
                if verbose:
                    print(f"No original found for {geom_name}, using identity matrix")
...
def export_scene_statistics(scene: Any, output_path: Path) -> None:
    ...
    try:
        ...
        print(f"Scene statistics exported to: {output_path}")
        
    except Exception as e:
        print(f"Error exporting scene statistics: {e}")
        raise
...
def export_scene_debug_info(scene: Any, output_dir: Path) -> List[Path]:
    ...
    try:
        ...
        print(f"Debug information exported to {len(created_files)} files in: {debug_dir}")
        return created_files
        
    except Exception as e:
        print(f"Error exporting scene debug info: {e}")
        raise
...
def visualize_grid(room_polygon, grid_size: float = DEFAULT_GRID_SIZE,
    ...
    if use_scene_objects:
                ...
                if hasattr(m, 'scene_objects') and m.scene_objects:
                    for so in m.scene_objects:
                        ...
                        if all(hasattr(so, attr) for attr in ['name', 'position', 'dimensions', 'rotation', 'object_type']):
                            ...
                            print(f"Warning: SceneObject in motif {m.id} is missing attributes. Skipping.")
                else:
                    print(f"Warning: Motif {m.id} has no 'scene_objects' attribute or it's empty. Skipping for scene_object visualization.")
        else:
            print("Warning: 'use_scene_objects' is True, but no motifs were provided.")
        ...
        for m in motifs:
            ...
            if all(hasattr(m, attr) for attr in ['id', 'position', 'extents', 'rotation', 'object_type']):
                ...
                print(f"Warning: Motif {getattr(m, 'id', 'unknown_id')} is missing attributes. Skipping.")
    ...
    if door_location is not None:
        ...
        if closest_wall_points:
            ...
            print(f"Warning: Could not determine wall segment for door location {door_location}")
        ...
        for window in window_location:
            ...
            if closest_wall_line:
                ...
                label_y = window_y + label_offset * math.sin(normal_angle_rad)
                 ...
                 print(f"Warning: Window location {window} not found on any wall segment.")
    ...
    if items_to_process: # Changed from "if motifs is not None:"
        ...
        for i, item in enumerate(items_to_process): # Changed from group to item, motifs to items_to_process
            ...
            if not (isinstance(item_extents, (list, tuple)) and len(item_extents) == 3):
                print(f"Warning: Item {item.id} has invalid extents format: {item_extents}. Skipping.")
                continue
    ...
    if items_to_process: # Changed from "if motifs is not None:"
        try:
            ...
            print(f"Error calculating free space occupancy: {e}")
