...
def extract_wall_data(room_polygon: Polygon, scene_motifs: list,
    ...
    if existing_wall_data:
        ...
        print("Using existing wall data")
    else:
    ...
    print(f"\nDebug: Window Processing - {len(window_locations) if window_locations else 0} windows")
    if window_locations:
        for window_idx, window_location in enumerate(window_locations):
            print(f"  Window {window_idx}: location {window_location}")
            ...
            for wall_idx, wall in enumerate(wall_data):
                ...
                if distance < 0.1:
                    ...
                    print(f"    Adding window to wall {wall_idx} at projection {projection:.2f}m")
                    wall_data[wall_idx]["blocked_areas"].append({
            ...
            if not window_placed:
                print(f"  Warning: Window {window_idx} at {window_location} could not be placed on any wall")
    else:
        print("  No window_locations provided to extract_wall_data function")

    # Process scene motifs for wall blocking
    print(f"\nDebug: Wall Detection - {len(scene_motifs)} motifs")

    for motif in scene_motifs:
        print(f"Checking motif: {motif.id}, Type: {motif.object_type}")
        ...
        if not include_wall_objects and is_wall_object:
            print(f"  Skipping wall object: {motif.id}")
            continue

        if not is_wall_object and motif.object_type != ObjectType.LARGE:
            print(f"  Skipping small object: {motif.id}")
            continue
        ...
        for wall_idx, wall in enumerate(wall_data):
            ...
            print(f"  Wall {wall_idx}: distance = {dist:.2f}m")
            ...
            if dist < detection_threshold:
                ...
                if is_wall_object:
                    ...
                    print(f"  Adding wall object to wall {wall_idx}: {proj_start:.2f}m - {proj_end:.2f}m at height {y:.2f}m")
                    wall_data[wall_idx]["blocked_areas"].append({
                        "start": proj_start,
                        "end": proj_end,
                        "height": height,
                    ...
                    if projections:
                        ...
                        print(f"  Adding blocked area on wall {wall_idx}: {min_proj:.2f}m - {max_proj:.2f}m")
                        wall_data[wall_idx]["blocked_areas"].append({
                            "start": min_proj,
                            "end": max_proj,
                            "height": height,
                        ...
                        print(f"  No valid projections on wall {wall_idx}")
    ...
    for wall in wall_data:
        ...
        for block in sorted_blocks:
            ...
            if block_width < min_width_threshold:
                print(f"  Small width block detected ({block_width:.2f}m): {processed_block['object_id']}")
        ...
        if processed_blocks:
            ...
            if processed_blocks[0]["start"] > 0 and processed_blocks[0]["start"] < edge_gap_threshold:
                print(f"  Small edge gap detected at wall start: {processed_blocks[0]['start']:.2f}m")
                gap_blocks.append({
                    "start": 0,
            ...
            for j in range(len(processed_blocks) - 1):
                ...
                if 0 < gap_size < edge_gap_threshold:
                    print(f"  Small gap detected between objects: {gap_size:.2f}m")
                    gap_blocks.append({
            ...
            if processed_blocks[-1]["end"] < wall_length and (wall_length - processed_blocks[-1]["end"]) < edge_gap_threshold:
                print(f"  Small edge gap detected at wall end: {(wall_length - processed_blocks[-1]['end']):.2f}m")
                gap_blocks.append({
        ...
        print(f"Wall {wall['id']}: Merged Blocked Length = {total_blocked_length:.2f}m, Available space = {available_percent:.1f}%")
...
def visualize_walls_as_surfaces(wall_data: list[dict], room_height: float = DEFAULT_ROOM_HEIGHT,
    ...
    print("\nDebug: Wall Surfaces Visualization")

    valid_wall_data = []
    if specific_wall_ids:
        print(f"Filtering for specific walls: {specific_wall_ids}")
        for wall in wall_data:
            if (isinstance(wall, dict) and
                ...
                print(f"Warning: Wall {wall.get('id')} is missing required data. Skipping.")
    else:
        print("Visualizing all walls with required data.")
        for wall in wall_data:
            if (isinstance(wall, dict) and
                "processed_blocks" in wall and
                "available_percent" in wall):
                ...
                print(f"Warning: Wall {wall.get('id', 'Unknown ID')} is missing required data. Skipping.")
            else:
                print(f"Warning: Invalid wall data item skipped: {wall}")


    if not valid_wall_data:
        print("Error: No valid walls with processed data found to visualize!")
        return None

    wall_data = valid_wall_data # Use only the valid walls
    num_walls = len(wall_data)
    print(f"Visualizing {num_walls} walls.")

    # Print summary of walls being visualized
    for i, wall in enumerate(wall_data):
        wall_id = wall.get("id", f"wall_{i}")
        print(f"Wall {i} ({wall_id}): Length={wall.get('length', 0):.2f}m, Processed Blocks: {len(wall.get('processed_blocks', []))}, Available: {wall.get('available_percent', 0):.1f}%")
    ...
    if output_path:
        plt.savefig(output_path, dpi=150, bbox_inches='tight')
        print(f"Wall surfaces visualization saved to {output_path}")
