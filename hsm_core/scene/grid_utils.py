...
def create_grid(room_polygon, grid_size=DEFAULT_GRID_SIZE, door_location=None):
    ...
    if door_location is None:
        ...
        if not perimeter_points:
            print("Warning: No perimeter points found. Using room centroid.")
            ...
            if not valid_points:
                print("Warning: No valid door locations found. Using a random perimeter point.")
                door_location = random.choice(perimeter_points)
            else:
                door_location = random.choice(valid_points)

        print(f"Door location set to: {door_location}")
    else:
...
def calculate_free_space(room_polygon, motifs):
    ...
    for motif in motifs:
        try:
            ...
            print(f"Error processing motif {motif.id}: {e}")
    ...
    try:
        ...
        print(f"Error in calculating occupancy: {e}")
        return 0.0
